...
{
  public class LocationWebsocketClientV2
  {
    private readonly int _port;
    public static int LocationV2Port { get; set; }
    private readonly Subject<Unit> _websocketErrorOccurred = new();
    private readonly Subject<LocationDataV2> _locationDataReceivedV2 = new();
    private ClientWebSocket _client;
    private CancellationTokenSource _tokenSource;


    public IObservable<Unit> ErrorOccurred => _websocketErrorOccurred;
    public IObservable<LocationDataV2> LocationDataReceivedV2 => _locationDataReceivedV2;

    public LocationWebsocketClientV2(int port)
    {
      _port = port;
    }
    ...
    private async Task ReadMessage()
    {
      ...
      try
      {
        var locationData = JsonConvert.DeserializeObject<LocationDataV2>(json);
        _locationDataReceivedV2.OnNext(locationData);
      }
      catch (Exception e)
      {
